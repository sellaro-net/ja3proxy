...
impl std::error::Error for SocksError {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match self {
            Self::ConnectFailed(error) | Self::DnsResolveFailure(error) => Some(&**error),
            ...
            let target_addr = match dns_resolve {
                DnsResolve::Local => {
                    let mut socket_addr = resolve(&mut resolver, Name::new(host.into()))
                        .await
                        .map(|mut s| s.find(|address| {
                            !matches!(version, Version::V4) || address.is_ipv4()
                        }))
                        .transpose()
                    ...
                    let stream = match auth {
                        Some((username, _)) => {
                            Socks4Stream::connect_with_userid_and_socket(
                                socket, target_addr, std::str::from_utf8(&username)?,
                            ).await?
                        }
                        None => Socks4Stream::connect_with_socket(socket, target_addr).await?,
                    };
