    ...
    T: AsyncRead + AsyncWrite + Unpin,
{
    ...
    loop {
        // A proxy response is untrusted; bound CONNECT headers independently of
        // application response streaming limits.
        if buf.len() >= 16 * 1024 {
            return Err(TunnelError::TunnelUnsuccessful);
        }
        let remaining = (16 * 1024 - buf.len()) as u64;
        if (&mut conn).take(remaining).read_buf(&mut buf).await.map_err(TunnelError::Io)? == 0 {
